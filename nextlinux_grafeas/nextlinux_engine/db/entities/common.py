...
def initialize(localconfig=None, versions=None, bootstrap_db=False, specific_tables=None, bootstrap_users=False):
    ...
    if bootstrap_db:
        from nextlinux_engine.db import db_nextlinux, db_users

        with session_scope() as dbsession:
            # version check
            version_record = db_nextlinux.get(session=dbsession)
            if not version_record:
                db_nextlinux.add(versions['service_version'], versions['db_version'], versions, session=dbsession)
                version_record = db_nextlinux.get(session=dbsession)

            if bootstrap_users:
                # system user
                try:
                    system_user_record = db_users.get('nextlinux-system', session=dbsession)
                    if not system_user_record:
                        rc = db_users.add('nextlinux-system', str(uuid.uuid4()), {'active': True}, session=dbsession)
                    else:
                        db_users.update(system_user_record['userId'], system_user_record['password'], {'active': True}, session=dbsession)

                except Exception as err:
                    raise Exception(
                        "Initialization failed: could not fetch/add nextlinux-system user from/to DB - exception: " + str(
                            err))
                ...
                try:
                    ...
                    for user_record in user_records:
                        if user_record['userId'] == 'nextlinux-system':
                            continue
...
def db_upgrade_001_002():
    global engine
    from nextlinux_engine.db import db_nextlinux, db_users, db_registries, db_policybundle, db_catalog_image
