...
from nextlinux_grafeas.nextlinux_engine.db import ImagePackage, ImagePackageVulnerability
...
@occurrence.command(name='package-vulnerabilities', short_help="Extract package-vulnerability occurrences from nextlinux engine DB")
...
def package_vulnerabilities(input_image_package):
    ...
    try:
        ...
        try:
            ...
            api_client = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.api_client.ApiClient(host=grafeas_hostport)
            api_instance = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.GrafeasApi(api_client=api_client)
...
def make_image_vulnerability_occurrence(imageId, anch_img_pkgs, dbsession=None, gapi=None):
    ...
    projects_id = "nextlinux-vulnerabilities"
    ...
    for anch_img_pkg in anch_img_pkgs:
        ...
        note_name = "projects/nextlinux-vulnerabilities/notes/"+anch_img_pkg.vulnerability_id
        severity = "UNKNOWN"
        cvss_score = 0.0
        ...
        affected_location = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.VulnerabilityLocation(
            package=pkgName,
            version=nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Version(kind="NORMAL", name=pkgFullVersion),
            cpe_uri="cpe:/a:"+anch_img_pkg.pkg_name+":"+anch_img_pkg.pkg_name+":"+anch_img_pkg.pkg_version
        )

        vulnerability_details = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.VulnerabilityDetails(
        ...
        newocc = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Occurrence(
            name='projects/nextlinux-vulnerability-scan/occurrences/'+str(occ_id),
...
def print_image_vulnerability_occurrences(gapi, img_id_set=None, pkg_name_set=None, vuln_id_set=None):
    ...
    with session_scope() as dbsession:
        if img_id_set:
            _logger.debug("fetching limited package set from nextlinux DB: " + str(img_id_set))
            try:
                dbfilter = {
        ...
        for imageId in anch_img_pkgs.keys():
            try:
                if list_only:
                    ...
                    #projects_id = "nextlinux-vulnerability-scan"
                    #note = gnote
