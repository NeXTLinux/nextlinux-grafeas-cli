...
import nextlinux_grafeas.cli.utils
from nextlinux_grafeas.cli.utils import session_scope
import nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client

from nextlinux_grafeas.nextlinux_engine.db import ImagePackage, Vulnerability
...
@note.command(name='packages', short_help="Extract package notes from nextlinux engine DB")
...
def packages(input_packages):
    ...
    nextlinux_grafeas.cli.utils.doexit(ecode)

@note.command(name='vulnerabilities', short_help="Extract vulnerability notes from nextlinux engine DB")
...
def make_package_note(pkgName, anch_pkgs):
    ...
    for anch_pkg in anch_pkgs:
        ...
        retel['latest_version'] = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Version(kind="NORMAL", name=anch_pkg.fullversion)
        ...
        dist = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Distribution(**retel)
        distributions.append(dist)
    
    package = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Package(name=pkgName, distribution=distributions)

    newnote = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Note(
        name="projects/nextlinux-distro-packages/notes/"+pkgName, 
...
def print_package_notes(pkg_name_set=[]):
    ...
    with session_scope() as dbsession:
        if pkg_name_set:
            _logger.debug("fetching limited package set from nextlinux DB: " + str(pkg_name_set))
            ...
            _logger.debug("fetching full package set from nextlinux DB")
...
def make_vulnerability_note(cveId, anch_vulns):
    ...
    for anch_vuln in anch_vulns:
        try:
            ...
            retel['min_affected_version'] = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Version(kind='MINIMUM')
            ...
            for fixedIn in anch_vuln.fixed_in:
                ...
                if fixedIn.version and fixedIn.version != "None":
                    fix_version = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Version(kind="NORMAL", name=fixedIn.epochless_version)
                else:
                    fix_version = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Version(kind="MAXIMUM")

                retel['fixed_location'] = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.VulnerabilityLocation(cpe_uri=retel['cpe_uri'], package=retel['package'], version=fix_version)

                detail = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Detail(**retel)
                vulnerability_details.append(detail)
        except Exception as err:
            _logger.warn("not enough info for detail creation - exception: " + str(err))
    
    vulnerability_type = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.VulnerabilityType(
    ...
    for link in links:
        external_urls.append(nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.RelatedUrl(url=link, label="More Info"))

    newnote = nextlinux_grafeas.vendored.grafeas_client.client_python.v1alpha1.swagger_client.Note(
        name="projects/nextlinux-vulnerabilities/notes/"+cveId, 
...
def print_vulnerability_notes(cve_id_set=[]):
    ...
    with session_scope() as dbsession:
        if cve_id_set:
            _logger.debug("fetching limited vulnerability set from nextlinux DB: " + str(cve_id_set))
            ...
            _logger.debug("fetching full vulnerability set from nextlinux DB")
            db_vulns = dbsession.query(Vulnerability).all()
            list_only = True

        for v in db_vulns:
            cveId = v.id
            if v.id not in nextlinux_vulns:
                nextlinux_vulns[v.id] = []
            nextlinux_vulns[v.id].append(v)

        for cveId in nextlinux_vulns.keys():
            try:
                if list_only:
                    print cveId
                else:
                    gnote = make_vulnerability_note(cveId, nextlinux_vulns[cveId])
                    print json.dumps(gnote.to_dict(), indent=4)
